import { Body, Injectable, Logger, NotFoundException, Patch } from "@nestjs/common";
...
export class CategoryService {
  ...
  async createCategory(createCategoryDto: CategoryDto): Promise<CategorySchema> {
    console.log(createCategoryDto,'****')
    try {
      // Create a new category document using create()
      const createdCategory = await this.categoryRepository.getRepository().create(createCategoryDto)
  ...
  async delete(id: string ): Promise<void> {
    ...
    try {
      await this.categoryRepository.getRepository().deleteOne({ _id: new Types.ObjectId(id)})
    } catch (error) {
 ...
 async updateCategory(categoryId: string, updateCategoryDto: Partial<CategoryDto>): Promise<CategorySchema> {
  try {
    const updatedCategory = await this.categoryRepository.getRepository().findByIdAndUpdate(
      categoryId,
