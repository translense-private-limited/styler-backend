...
export class RolePermissionService {
  constructor(private readonly rolePermissionMappingRepository: RolePermissionMappingRepository) {}

  // Main function to update role permissions based on incoming data
 
  async updateRolePermissions(rolePermissionMappingDto: RolePermissionMappingDto): Promise<void> {
    ...
    const resourceIds = resourcePermissions.map(resource => resource.resourceId);

    // Step 2: Fetch existing mappings for the specified roleId and resourceIds in a single query
    const existingMappings = await this.rolePermissionMappingRepository.findRoleResourceMappings(roleId, resourceIds);

    // Step 3: Create an object to store existing mappings by resourceId for quick lookup
    const existingMappingMap = Object.fromEntries(existingMappings.map(mapping => [mapping.resourceId, mapping]));
    ...
    const updatedMappings = Object.values(existingMappingMap).concat(newMappings);

    // Step 7: Save all updated and new mappings in one go
    if (updatedMappings.length > 0) {
        await this.rolePermissionMappingRepository.getRepository().save(updatedMappings);
    }
}
