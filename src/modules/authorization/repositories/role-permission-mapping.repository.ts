...
export class RolePermissionMappingRepository extends BaseRepository<RolePermissionMappingEntity> {
  ...
  async getRolePermissions(roleId: number): Promise<ResourcePermissionDto[]> {
    const roleResourceMapping = await this.repository.find({
      where: { roleId },
    });
  ...
  async findRoleResourceMappings(
    roleId: number,
    resourceIds: number[],
  ...
  async saveUpdatedMappings(
    mappings: RolePermissionMappingEntity[],
  ): Promise<void> {
